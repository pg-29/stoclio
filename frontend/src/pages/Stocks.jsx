...
function normalize(item) {
  return { ...item, symbol: item.tradingsymbol || item.symbol, price: Number(item.ltp ?? item.last_traded_price), open: Number(item.open ?? item.open_price_day), volume: Number(item.tradeVolume ?? item.volume ?? item.trade_volume), changePercent: Number(item.percentChange ?? item.changePercent) };
}
